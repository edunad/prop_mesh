...
ENT.IGNORE_CONTENT_RANGE = GetConVar( "prop_mesh_ignoreContentRange" )
...
function ENT:CheckOBJUri(uri, onComplete)
	if self.IGNORE_CONTENT_RANGE:GetInt() ~= 0 then
		print("[PropMLIB] Skipping content range check")
	...
	PropMLIB.MeshParser.Register(self, {
		onInitialize = function(onInitialized)
			...
			self:CheckOBJUri(uri, function(err, data)
				...
				if not data then
					PropMLIB.MeshParser.QueueDone()
					return onFail("Failed to parse model")
				end
				...
				HTTP({
					...
					success = function(code, body, headers)
						if code ~= 200 then
							PropMLIB.MeshParser.QueueDone()
							return onFail("!! Failed to fetch model: "..code.." !!")
						end
