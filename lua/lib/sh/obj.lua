...
PropMLIB.Obj.MAX_SAFE_TRIANGLES = GetConVar( "prop_mesh_maxTriangles" )
...
PropMLIB.Obj.Parse = function(isAdmin, body, fixNormals)
	...
	local maxTriangles = PropMLIB.Obj.MAX_SAFE_TRIANGLES:GetInt()
	...
	for _, v in pairs(rawData) do
		...
		elseif CLIENT then
			...
			elseif mode == "f" then -- FACES
				...
				if subMeshes[#subMeshes].faceCount >= maxTriangles then
					if #subMeshes < maxSubMeshes or isAdmin then
						print("[PropMLIB][Client] Triangle limit, splitting model..")

						local splitName = subMeshes[#subMeshes].name or "obj"
						if string.find(splitName, "_split") then
							table_insert(subMeshes, PropMLIB.Obj.NewSubMesh(splitName))
						else
							table_insert(subMeshes, PropMLIB.Obj.NewSubMesh(splitName.."_split"))
						end
					end
				else
					subMeshes[#subMeshes].faceLines[#subMeshes[#subMeshes].faceLines + 1] = parts
					subMeshes[#subMeshes].faceCount = subMeshes[#subMeshes].faceCount + 1
				end
